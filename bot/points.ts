...
export class Points{
    ...
    constructor(client:discordClient, sql:SqlQueries){
        this.sql = sql
        this.client = client
        this.openedSessions = []
        this.operations = []
    ...
    async init(){
        ...
        let guild = this.client.guilds.cache.find(e=>e.id == guildId)
        if(!guild){
        ...
        this.client.on("voiceStateUpdate", async (oldState, newState)=>{
            ...
            this.operations.push({user:oldState.member.id, timeoutCode:setTimeout(()=>{
                if(oldState.guild.id != guildId )return//mauvaise guild
